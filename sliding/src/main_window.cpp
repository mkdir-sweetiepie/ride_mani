...
void MainWindow::slotUpdateImg()
{
  ...
  // labeling(green_img, drawimg);
  // QImage draw_image(drawimg.data, drawimg.cols, drawimg.rows, drawimg.step, QImage::Format_RGB888);  // 세번째 UI
  // ui.draw->setPixmap(QPixmap::fromImage(draw_image));
                                  ...
                                  int maxHeight)
{
  ...
  {
    qDebug() << "Image dimensions are not larger than 130x130!";
  }
}
/*****************************************************************************
 ** 자율주행 :
